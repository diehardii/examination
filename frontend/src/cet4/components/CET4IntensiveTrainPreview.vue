  ...
  <div class="question-container">
    ...
    <div class="main-content">
      <div class="question-wrapper">
        ...
        <div v-if="listeningUnitsAB.length" class="result-card structured exam-section">
          <div class="section-header">
            ...
            <div class="audio-controls">
              ...
              >
                <span v-if="!audioGenerating">🔊 生成听力音频</span>
              ...
              <div v-if="hasAudio && !audioGenerating" class="audio-player">
                ...
                <button @click="stopAudio" class="stop-btn">⏹️ 停止</button>
              </div>
            </div>
          </div>
          ...
          >
            <div class="passage-header">
              <h3>{{ displayUnitType(unit.unit_type) }}</h3>
            </div>
        ...
        <!-- 保存试卷区域（仅在强化训练临时模式下显示，错题再训练不显示） -->
        <div v-if="!loading && !errorMessage && isIntensiveTrainTemp && !isWrongQuestionRetry" class="save-paper-section">
          <div v-if="!savedPaperId" class="save-controls">
            <button class="save-paper-btn" @click="openSaveDialog">保存试卷</button>
        ...
        <!-- 显示条件：
             ...
             3. 正常模式：直接显示
        -->
        <div class="submit-btn-container" v-if="!loading && !errorMessage && (isWrongQuestionRetry || !isIntensiveTrainTemp || savedPaperId)">
          <el-progress
            v-if="isSubmitting"
    ...
    <CET4SaveMetaModal
      ...
      @update:description="val => (saveForm.description = val)"
...
export default {
  ...
  props: {
    examPaperEnId: {
      type: [String, Number],
      default: '',
      required: false  // 不再是必需的，优先从 sessionStorage 读取
    }
  },
  setup(props) {
    ...
    // 保存试卷相关状态
    const isIntensiveTrainTemp = ref(false)  // 是否是强化训练临时模式
    const isWrongQuestionRetry = ref(false)  // 是否是错题再训练模式
    const savedPaperId = ref(null)           // 保存后的试卷ID
    const isSaving = ref(false)              // 是否正在保存（API）
    const saveError = ref('')                // 保存错误信息
    const saveSuccess = ref('')              // 保存成功信息
    const originalTrainData = ref(null)      // 原始的训练数据（用于保存）
    const showSaveDialog = ref(false)        // 是否显示保存弹窗
    const savingPaperInfo = ref(false)       // 弹窗按钮 loading
    ...
    // 规范化听力单元标题：去掉为排序/标号附加的数字后缀，保留原始类型名称
    const displayUnitType = (raw) => {
      const label = (raw || '').toString().trim()
      if (!label) return '听力单元'
      const patterns = [
        /^news\s*report\s*[-–]?\s*\d+$/i,
        /^conversation\s*[-–]?\s*\d+$/i,
        /^listening\s*passage\s*[-–]?\s*\d+$/i,
      ]
      const baseNames = {
        news: 'News Report',
        conversation: 'Conversation',
        listeningpassage: 'Listening Passage'
      }
      for (const p of patterns) {
        if (p.test(label)) {
          const key = label.toLowerCase().replace(/\s|[-–]|\d/g, '')
          return baseNames[key] || label.replace(/[-–]?\s*\d+$/,'').trim()
        }
      }
      return label
    }
    ...
    onMounted(async () => {
      ...
      try {
        // 检查是否是错题再训练模式（从 query 参数判断）
        const fromWrongQuestion = route.query.from_wrong_question === 'true'
        
        // 先检查是否有强化训练的临时数据（从sessionStorage读取）
        const intensiveTrainData = sessionStorage.getItem('intensiveTrainQuestions')
        
        // 如果有强化训练数据，优先使用（不需要examPaperEnId）
        if (intensiveTrainData) {
          // 判断是错题再训练还是普通强化训练
          if (fromWrongQuestion) {
            isWrongQuestionRetry.value = true
          } else {
            isIntensiveTrainTemp.value = true
          }
          ...
          // 保存原始数据，用于后续保存操作
          originalTrainData.value = trainData
          ...
          const convertedUnits = (trainData.questions || []).map((q, index) => {
            ...
            // 提取正确答案到 correctAnswers（用于错题再训练模式）
            
            if (parsedDoc && parsedDoc.answers) {
              const segmentId = q.segmentIdSelf || `${index + 1}${q.questionType}`
              
              // 遍历 answers 数组，提取每个题目的正确答案
              parsedDoc.answers.forEach(answerObj => {
                // 兼容 questionNumber 和 question_number 两种字段名
                const questionNum = answerObj.questionNumber || answerObj.question_number
                if (questionNum && answerObj.answer !== undefined) {
                  const key = `${segmentId}-${questionNum}`
                  correctAnswers.value[key] = answerObj.answer
                } else {
                }
              })
            } else {
            ...
            return {
              ...
              metadata: {
                ...
                exam_paper_en_name: trainData.paperName || '强化训练试卷（未保存）'
              }
            }
          })
          // 将转换后的数据赋值给 units
          ...
          examPaperName.value = trainData.paperName || '强化训练试卷（未保存）'
        } else if (props.examPaperEnId) {
          // 正常模式：使用examPaperId查询ChromaDB（当有examPaperEnId时）
          ...
          // 既没有sessionStorage数据，也没有examPaperEnId
          throw new Error('缺少试卷数据：请从强化训练页面生成试卷或提供试卷ID')
        }
      } catch (error) {
    ...
    const parseUnits = () => {
      ...
      if (sectionABUnits.length > 0) {
        ...
        sectionABUnits.forEach(unit => {
            ...
            if (obj) {
              const unit_type = (obj?.unit_type || obj?.unitType || obj?.type || '').trim()
              const listening_content = (obj?.listening_content || obj?.content || obj?.passage || '').trim()

              // 兼容不同字段名的题目数组
              let questionsRaw = []
              if (Array.isArray(obj?.question_and_options)) {
                questionsRaw = obj.question_and_options
              } else if (Array.isArray(obj?.questions)) {
                questionsRaw = obj.questions
              } else if (Array.isArray(obj?.question_list)) {
                questionsRaw = obj.question_list
              }

              // 归一化题目字段名，确保 question_number 存在
              const questions = (questionsRaw || []).map(q => ({
                ...q,
                question_number: q.question_number || q.questionNumber || q.qno || q.no || q.index,
                question_content: q.question_content || q.question || q.text || q.stem,
                options: q.options || q.option_list || q.question_and_options || q.option_items
              }))
            ...
            if (Array.isArray(obj?.answers) && obj.answers.length > 0) {
              obj.answers.forEach(ans => {
                if (ans.question_number && ans.answer) {
                  ...
                  correctAnswers.value[ans.question_number] = ans.answer // 兼容性
                }
              })
            }
            // 如果 question_and_options 中也有答案，也提取（正常模式）
            if (Array.isArray(questions)) {
              questions.forEach(q => {
                if (q.question_number && q.answer) {
                  const answerKey = `${segment_id}-${q.question_number}`
                  correctAnswers.value[answerKey] = q.answer
                  correctAnswers.value[q.question_number] = q.answer // 兼容性
                }
              })
            }
      ...
      // Section C - 放宽过滤，识别 ReadingPassage 题型
      const sectionCUnits = part3Units.filter(u => {
        const meta = u.metadata || {}
        const segmentId = (meta.segment_id || '').toString()
        const qt = (meta.question_type || '').toString()
        
        // 1. 题型匹配：ReadingPassage 或 篇章阅读
        const isReadingType = qt === 'ReadingPassage' || qt === '篇章阅读' || qt === 'ReadingComprehension'
        
        // 2. segment_id 匹配：C1/C2/readingpassage/包含数字+ReadingPassage
    ...
    const savePaper = async () => {
      ...
      if (!name) {
        saveFormError.value = '请输入试卷名称'
        saveError.value = '请输入试卷名称'
        saveSuccess.value = ''
        return
      }
      
      if (!originalTrainData.value || !originalTrainData.value.questions) {
        saveFormError.value = '没有可保存的题目数据'
        saveError.value = '没有可保存的题目数据'
        saveSuccess.value = ''
        return
      }
      ...
      try {
        const payload = {
          paperName: name,
          questionsData: originalTrainData.value.questions,
          exam_paper_en_source: originalTrainData.value.examPaperEnSource,
          paperDesc: description || undefined
        }
        
        const { data } = await axios.post('/api/intensive/save', payload)
        
        if (data && data.paperId) {
          savedPaperId.value = data.paperId
          examPaperId.value = data.paperId
          examPaperName.value = name
          saveSuccess.value = '试卷保存成功！'
          saveError.value = ''
          saveFormError.value = ''
          ...
          // 显示成功提示
          ElMessage.success({
            message: `试卷"${name}"保存成功！`,
            duration: 3000
          })
          
          // 清除 sessionStorage 中的临时数据（因为已经保存到数据库）
          sessionStorage.removeItem('intensiveTrainQuestions')
        } else {
          throw new Error('保存失败：未返回试卷ID')
        }
      } catch (err) {
        const errMsg = '保存试卷时出错: ' + (err.message || '未知错误')
        saveFormError.value = errMsg
    ...
    const openSaveDialog = () => {
      saveForm.value.name = examPaperName.value || ''
    ...
    const startSubmitProgress = () => {
      ...
      submitProgressInterval.value = setInterval(() => {
        submitProgress.value = Math.min(submitProgress.value + step, 99) // 预留收尾
      }, 1000)
    }
    ...
    const submitAnswers = async () => {
      ...
      try {
        ...
        // 错题再训练模式：不保存到数据库，直接本地评分
        if (isWrongQuestionRetry.value) {
          // 展平答案对象，将嵌套的答案合并到一个对象中
          const flattenedAnswers = {}
          
          // 合并所有部分的答案
          if (allAnswers.writing && typeof allAnswers.writing === 'object') {
            Object.assign(flattenedAnswers, allAnswers.writing)
          }
          if (allAnswers.translation && typeof allAnswers.translation === 'object') {
            Object.assign(flattenedAnswers, allAnswers.translation)
          }
          if (allAnswers.listening && typeof allAnswers.listening === 'object') {
            Object.assign(flattenedAnswers, allAnswers.listening)
          }
          if (allAnswers.sectionA && typeof allAnswers.sectionA === 'object') {
            Object.assign(flattenedAnswers, allAnswers.sectionA)
          }
          if (allAnswers.sectionB && typeof allAnswers.sectionB === 'object') {
            Object.assign(flattenedAnswers, allAnswers.sectionB)
          }
          if (allAnswers.sectionC && typeof allAnswers.sectionC === 'object') {
            Object.assign(flattenedAnswers, allAnswers.sectionC)
          }
          
          // 将答案和试卷数据存储到 sessionStorage，供结果页面使用
          const tempResultData = {
            mode: 'wrong_question_retry',
            answers: flattenedAnswers,  // 使用展平后的答案
            correctAnswers: correctAnswers.value,
            units: units.value,
            examPaperEnSource: examPaperEnSource.value,
            examPaperName: examPaperName.value,
            timestamp: Date.now()
          }
          sessionStorage.setItem('tempTestResult', JSON.stringify(tempResultData))
        ...
        // 强化训练模式：保存到数据库
        const paperIdToSubmit = savedPaperId.value || examPaperId.value
        
        if (!paperIdToSubmit || paperIdToSubmit === 'temp') {
          ElMessage.error('试卷ID无效，请先保存试卷')
          stopSubmitProgress()
          return
        }
        ...
        const response = await axios.post('http://localhost:8080/api/exam-paper/submit-answer-en', {
          examPaperEnId: paperIdToSubmit,
          answers: allAnswers,
          correctAnswers: correctAnswers.value,
          exam_paper_en_source: examPaperEnSource.value
        }, {
          withCredentials: true,
          timeout: 1800000 // 提交批改流程耗时可能较长，放宽至30分钟
        })
    ...
    return {
      ...
      examPaperEnSource,
      // 保存试卷相关
      isIntensiveTrainTemp,
      isWrongQuestionRetry,
      savedPaperId,
      isSaving,
      saveError,
      saveSuccess,
...
/* Modal styles (aligned with PaperGen) */
.modal-mask {
  position: fixed;
  z-index: 999;
  top: 0;
...
.save-paper-btn:hover:not(:disabled) {
  background-color: #45a049;
  box-shadow: 0 4px 8px rgba(76, 175, 80, 0.3);
  transform: translateY(-1px);
}

.save-paper-btn:active:not(:disabled) {
  transform: translateY(0);
}

.save-paper-btn:disabled {
