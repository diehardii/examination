...
@Mapper
public interface CET4EExamPaperEnMapper {
    ...
    int insert(CET4ExamPaperEn examPaperEn);
    ...
    List<CET4ExamPaperEn> selectAll();
    
    @Select("SELECT id, exam_paper_en_name as examPaperEnName, exam_paper_en_desc as examPaperEnDesc, " +
            "exam_paper_en_subject as examPaperEnSubject, exam_paper_en_source as examPaperEnSource FROM exam_paper_en WHERE id = #{id}")
    CET4ExamPaperEn getExamPaperEnById(@Param("id") Long id);
    
    @Select("SELECT id FROM exam_paper_en WHERE exam_paper_en_subject = #{subject}")
    List<Long> findIdsBySubject(@Param("subject") String subject);

    // 新增：按来源查询ID
    @Select("SELECT id FROM exam_paper_en WHERE exam_paper_en_source = #{source}")
